...
def excel2lua(src_excel_path, tgt_lua_path):
    ...
    # print('[file] %s -> %s' % (os.path.basename(src_excel_path), os.path.basename(tgt_lua_path)))
    # load excel data
    excel_data_src = xlrd.open_workbook(
        src_excel_path, encoding_override='utf-8')
    ...
    for row in range(4, excel_sheet.nrows):
        ...
        if cell_id.value in excel_data_dict:
            print('[warning] duplicated data id: "%d", all previous value will be ignored!~' % (
                cell_id.value))
    ...
    # file head desc
    lua_file_head_desc = lua_file_head_format_desc % (
        tgt_lua_file_name, src_excel_file_name)

    # export to lua file
    # encoding='utf-8'储存为utf-8格式
    ...
    print('[excel] %d row data exported !~ %s' %
          (excel_sheet.nrows, os.path.basename(tgt_lua_path)))
...
if __name__ == '__main__':
    ...
    excel2lua(os.path.join(curpath, sys.argv[1]), os.path.join(
        curpath, sys.argv[2]))
