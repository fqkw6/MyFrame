...
namespace CircularScrollView
{
    ...
    public class UICircularScrollView : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler
    {
        ...
        public virtual void UpdateList()
        {
            ...
            {
                Debug.LogError(m_CellInfos[i].obj);
        ...
        public virtual void ShowList(int num)
        {
            ...
            if (m_Direction == e_Direction.Vertical)
            {
                float contentSize = (m_Spacing + m_CellObjectHeight) * Mathf.CeilToInt((float)num / m_Colum);
            ...
            {
                float contentSize = (m_Spacing + m_CellObjectWidth) * Mathf.CeilToInt((float)num / m_Colum);
            ...
            for (int i = 0; i < num; i++)
            {
                ...
                if (m_Direction == e_Direction.Vertical)
                {
                    pos = m_CellObjectHeight * Mathf.FloorToInt(i / m_Colum) + m_Spacing * Mathf.FloorToInt(i / m_Colum);
                    rowPos = m_CellObjectWidth * (i % m_Colum) + m_Spacing * (i % m_Colum);
                    cellInfo.pos = new Vector3(rowPos, -pos, 0);
                }
                else
                {
                    pos = m_CellObjectWidth * Mathf.FloorToInt(i / m_Colum) + m_Spacing * Mathf.FloorToInt(i / m_Colum);
        ...
        {
            float a = Math.Abs(m_current - index) / (float)index;
            LocateRenderItemAtIndex(index - 1, a);
        }
        #region 移动至数据
        /// <summary>
        /// 移动列表使之能定位到给定数据的位置上
        ...
        {
            if (index < 0 || index > m_CellInfos.Length - 1)
                throw new Exception("Locate Index Error " + index);

            index = Math.Min(index, m_CellInfos.Length - m_MaxIndex + 1);
            index = Math.Max(0, index);
            Vector2 pos = m_ContentRectTrans.anchoredPosition;
            int row = index / m_Colum;
        ...
        {
            return m_CellObjectHeight + m_Spacing;
        }
