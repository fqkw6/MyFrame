...
---通过新图集加载
local function SetSpriteNameBySpriteAtlas(self, sprite_atlas, sprite_name)
    self.sprite_name = sprite_name
    if IsNull(self.unity_uiimage) then
        return
    end
    ...
    AtlasManager:GetInstance():LoadImageAsyncByAtlas(
        ...
        function(atlas)
            ...
            local sprite = atlas:GetSprite(sprite_name)
            -- 被加载的Sprite不是当前想要的Sprite：可能预设被复用，之前的加载操作就要作废
...
UIImage.SetSpriteNameBySpriteAtlas = SetSpriteNameBySpriteAtlas
return UIImage
