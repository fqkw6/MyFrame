        ...
        public override void ShowList(string numStr)
        {
            ...
            for (int k = 0; k < buttonCount; k++)
            {
                ...
                if (m_Direction == e_Direction.Vertical)
                {
                    ...
                    pos += k > 0 ? (m_CellObjectHeight + m_Spacing) * Mathf.CeilToInt((float)beforeCellCount / m_Colum) : 0;
                ...
                else
                {
                    ...
                    pos += k > 0 ? (m_CellObjectWidth + m_Spacing) * Mathf.CeilToInt((float)beforeCellCount / m_Colum) : 0;
                ...
                expandInfo.size = m_Direction == e_Direction.Vertical ? (m_CellObjectHeight + m_Spacing) * Mathf.CeilToInt((float)count / m_Colum) : (m_CellObjectWidth + m_Spacing) * Mathf.CeilToInt((float)count / m_Colum); //计算 需展开的尺寸
                ...
                for (int i = 0; i < count; i++)
                {
                    ...
                    if (m_Direction == e_Direction.Vertical)
                    {
                        pos = m_CellObjectHeight * Mathf.FloorToInt(i / m_Colum) + m_Spacing * (Mathf.FloorToInt(i / m_Colum) + 1);
                        pos += (m_ExpandButtonHeight + m_Spacing) * (k + 1);
                        pos += (m_CellObjectHeight + m_Spacing) * Mathf.CeilToInt((float)beforeCellCount / m_Colum);
                        rowPos = m_CellObjectWidth * (i % m_Colum) + m_Spacing * (i % m_Colum);
                        cellInfo.pos = new Vector3(rowPos, -pos, 0);
                    }
                    else
                    {
                        pos = m_CellObjectWidth * Mathf.FloorToInt(i / m_Colum) + m_Spacing * (Mathf.FloorToInt(i / m_Colum) + 1);
                        pos += (m_ExpandButtonWidth + m_Spacing) * (k + 1);
                        pos += (m_CellObjectHeight + m_Spacing) * Mathf.CeilToInt((float)beforeCellCount / m_Colum);
                        rowPos = m_CellObjectHeight * (i % m_Colum) + m_Spacing * (i % m_Colum);
            ...
            if (!m_IsInited)
            {
                ...
                if (m_Direction == e_Direction.Vertical)
                {
                    float contentSize = m_IsExpand ? (m_Spacing + m_CellObjectHeight) * Mathf.CeilToInt((float)totalCount / m_Colum) : 0;
                ...
                {
                    float contentSize = m_IsExpand ? (m_Spacing + m_CellObjectWidth) * Mathf.CeilToInt((float)totalCount / m_Colum) : 0;
        ...
        public override void OnClickExpand(int index)
        {
            ...
            for (int k = 0, length = m_ExpandInfos.Length; k < length; k++)
            {
                ...
                if (k >= index)
                {
                    ...
                    {
                        pos = m_ExpandButtonWidth * k + m_Spacing * (k + 1);
                        pos += (m_CellObjectWidth + m_Spacing) * Mathf.CeilToInt((float)beforeCellCount / m_Colum);
                        button.transform.GetComponent<RectTransform>().anchoredPosition = new Vector3(pos, m_ExpandButtonY, 0);
                    }
